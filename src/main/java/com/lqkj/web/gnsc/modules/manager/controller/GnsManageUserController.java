...
@RestController
@Validated
public class GnsManageUserController {

    @Autowired
    GnsManageUserService gnsManageUserService;

    @ApiOperation("注册用户")
    @PutMapping("/center/user/register")
    public MessageBean<GnsManageUser> register(@RequestBody GnsManageUser user, @RequestParam Integer adminCode,
    ...
    public MessageBean<GnsManageUser> info(@PathVariable Long id,
    ...
    public MessageBean<GnsManageUser> info(@PathVariable String username, @ApiIgnore Authentication authentication) throws Exception {
        ...
        return MessageBean.ok((GnsManageUser) gnsManageUserService.loadUserByUsername(name));
    }

    @ApiOperation("更新用户密码")
    @PostMapping("/center/user/{id}")
    public MessageBean<GnsManageUser> update(@RequestParam(required = false) String password,
...
//    }
    ...
    public MessageBean<Page<GnsManageUser>> page(String keyword, Integer page, Integer pageSize, @ApiIgnore Authentication authentication) {
        String name = "";
        if (authentication != null) {
            Jwt jwt = (Jwt) authentication.getPrincipal();
            name = (String) jwt.getClaims().get("user_name");
    ...
    public MessageListBean<GnsManageRole> ruleByUserId(@PathVariable Long id, @ApiIgnore Authentication authentication) {
        String name = "";
        if (authentication != null) {
            Jwt jwt = (Jwt) authentication.getPrincipal();
            name = (String) jwt.getClaims().get("user_name");
    ...
    public MessageBean<GnsManageUser> oauth(@ApiIgnore Authentication authentication) {
        ...
        return MessageBean.ok((GnsManageUser) gnsManageUserService.loadUserByUsername(userCode));
