...
import com.lqkj.web.gnsc.modules.manager.dao.GnsManageUserBatchRepository;
...
@Service
@Transactional
public class GnsManageUserService implements UserDetailsService {
    ...
    @Autowired
    GnsManageUserBatchRepository userBatchRepository;
    ...
    public GnsManageUser findByUserName(String userName){
     ...
     * 注册管理员用户
     */
    public GnsManageUser registerAdmin(Integer adminCode, GnsManageUser user, String userName) throws Exception {
        ...
        user.setUserGroup(GnsManageUser.UserGroupType.teacher_staff);
     ...
     * 查询用户信息
     */
    public GnsManageUser info(Long id, String userName) {
     ...
     * 更新用户密码和头像
     */
    public GnsManageUser update(Long id, String password,String oldPassword ,Boolean admin,String headPath,String userName) throws UserPrincipalNotFoundException {
        logService.addLog("用户管理服务", "update",
                "更新用户密码和头像", userName);

        GnsManageUser user = userRepository.findById(id).get();
        //密码验证
     ...
     * 分页搜索
     */
    public Page<GnsManageUser> page(String keyword, Integer page, Integer pageSize, String username) {
        logService.addLog("用户管理服务", "page",
                "分页查询用户列表", username);

        GnsManageUser user = new GnsManageUser();
        ...
        Page<GnsManageUser> result = userRepository.findAll(Example.of(user, exampleMatcher),
                PageRequest.of(page, pageSize));
        return result.map(v -> (GnsManageUser) this.setIconURL(v));
    }

    /**
     * 查询用户角色
     */
    public Set<GnsManageRole> findRulesByUserId(Long id, String name) {
    ...
    public void bindRules(Long userId, Long[] rules, String userCode) {
        ...
        GnsManageUser user = userRepository.getOne(userId);
    ...
    public void loginout(Long userId) {
        GnsManageUser savedUser = userRepository.getOne(userId);
     ...
     * @return
     */
    public GnsManageUser findByUserCode(String userCode){
...
//                        user.setUserCode(userCode);
//                        user.setAdmin(false);
//                        user.setUserGroup(GnsManageUser.UserGroupType.student);
//                        user.setUserName(jsonNode.get("realName").textValue());
//                        user.setPassWord(password);
...
//    }
    ...
    /*private void executeSql(StringBuffer userCodeString, StringBuffer userGroupString,StringBuffer userRuleString,String password){
        ...
        GnsManageUserBatchRepository.bulkMergeUser(sqlString.toString());
    }*/
     ...
     * 设置图片访问地址
     */
    public GnsManageUser setIconURL(GnsManageUser user) {
