...
import com.lqkj.web.gnsc.modules.manager.service.GnsManageResourceService;
...
@RestController
@Validated
public class GnsManageResourceController {

    @Autowired
    GnsManageResourceService resourceService;

    @ApiOperation("增加用户权限")
    @PutMapping("/center/user/authority/")
    public MessageBean<GnsManageResource> add(GnsManageResource authority,
    ...
    public MessageBean<GnsManageResource> update(@RequestBody GnsManageResource authority,
    ...
    public MessageBean<GnsManageResource> info(@PathVariable Long id,
                                                                                       @ApiIgnore Authentication authentication) {
        Jwt jwt = (Jwt)authentication.getPrincipal();
        String userName = (String)jwt.getClaims().get("user_name") == null? "guest" : (String)jwt.getClaims().get("user_name");
    ...
    public MessageBean<Page<GnsManageResource>> page(String keyword,
    ...
    public MessageListBean<GnsManageResource> queryByRule(@RequestParam Long ruleId,
                                                          @ApiIgnore Authentication authentication) {
        Jwt jwt = (Jwt)authentication.getPrincipal();
        String userName = (String)jwt.getClaims().get("user_name") == null? "guest" : (String)jwt.getClaims().get("user_name");
    ...
    public MessageListBean<GnsManageResource> queryByType(@PathVariable GnsManageResource.UserAuthorityType type,
                                                                                                  @ApiIgnore Authentication authentication) {
        Jwt jwt = (Jwt)authentication.getPrincipal();
        String userName = (String)jwt.getClaims().get("user_name") == null? "guest" : (String)jwt.getClaims().get("user_name");
    ...
    public MessageBean<List<GnsManageResource>> findByRoleAndUserId(
            @ApiIgnore Authentication authentication) {
        String rules = "";
        String userCode = "";
        if(authentication != null){
