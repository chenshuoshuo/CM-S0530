...
public interface PointTypeDao extends JpaRepository<GnsDisplayPointType, GnsDisplayPointTypePK> {
    ...
    @Query(nativeQuery = true,
        value = "with t1 as(select p.point_type_code ,t.type_name,t.vector_icon from gns.gns_display_point_type p left join portal.map_point_type t on p.point_type_code = t.type_code where p.school_id =:schoolId)\n" +
                " select p.type_code typeCode,p.type_name typeName,t1.vector_icon vectorIcon from portal.map_point_type p,t1 where p.type_code = t1.point_type_code")
