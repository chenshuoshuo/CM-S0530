...
import com.lqkj.web.gnsc.modules.manager.service.GnsManageRoleService;
...
@RestController
@Validated
public class GnsManageRoleController {

    @Autowired
    GnsManageRoleService roleService;

    @ApiOperation("增加角色")
    @PutMapping("/center/user/rule")
    public MessageBean<GnsManageRole> add(@RequestParam String name,
        ...
        if(object!=null){
            return MessageBean.ok((GnsManageRole) object);
        }
    ...
    public MessageBean<GnsManageRole> update(@RequestParam String name,
                                                                                     @RequestParam String enName,
                                                                                     @RequestParam Long[] authorities,
    ...
    public MessageListBean<GnsManageRole> updateAuthorities(@RequestParam String[] roles,
                                                            @RequestParam Long[] authorities) {
    ...
    public MessageBean<GnsManageRole> info(@PathVariable Long id) {
        return MessageBean.ok(roleService.info(id));
    }

    @ApiOperation("分页查询用户角色")
    @GetMapping("/center/user/rule")
    public MessageBean<Page<GnsManageRole>> page(@RequestParam(required = false) String keyword,
    ...
    public MessageListBean<GnsManageRole> ruleAll() {
