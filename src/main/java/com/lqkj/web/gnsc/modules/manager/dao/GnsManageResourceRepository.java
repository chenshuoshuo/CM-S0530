...
@Repository
public interface GnsManageResourceRepository extends JpaRepository<GnsManageResource, Long> {

    @Query(nativeQuery = true, value = "select a.* from gns_manage_resource a" +
    ...
    List<GnsManageResource> findByManageType(@Param("userCode") String userCode,
                                                                                     @Param("t") String type);
    ...
    List<GnsManageResource> findByType(@Param("userCode") String userCode, @Param("t") String type);


    @Query("select a.name from GnsManageResource a where upper(a.content)=upper(:content) ")
    String findNameByContent(@Param("content") String content);

    @Modifying
    @Query("update GnsManageResource a set a.enabled=:enabled where a.authorityId=:authorityId")
    void updateChildState(@Param("authorityId") Long authorityId,
                          @Param("enabled") Boolean enabled);

    @Query(nativeQuery = true, value = "select a.* from gns.gns_manage_resource a" +
            " inner join gns.gns_manage_role_resource ra on a.authority_id = ra.authority_id " +
            " inner join gns.gns_manage_role r on ra.rule_id = r.rule_id " +
            " inner join gns.gns_manage_user_role ur on r.rule_id = ur.rule_id " +
            " inner join gns.gns_manage_user u on ur.user_id = u.user_id " +
            " where u.user_code=:userName and a.name like :keyword group by a.authority_id")
    Page<GnsManageResource> findSupportAuthority(@Param("userName") String userName,
                                                                                         @Param("keyword") String keyword,
                                                                                         Pageable pageable);
    @Query("select a from GnsManageResource a where a.parentId=:id")
    List<GnsManageResource> queryChildAuth(Long id);
